---
title: "Farming Creator"
description: "Documentation officielle du script FiveM Farming Creator, incluant le lien vers le fil du forum et la boutique."
icon: "tractor"
---

Voici la documentation officielle de **Farming Creator**.

<CardGroup cols={2}>
  <Card title="FiveM Forum Post" icon="comments" href="https://forum.cfx.re/t/farming-creator-esx-qbcore-in-game-farming-creation-menu/4954237">
    Fil officiel de sortie sur le forum FiveM.
  </Card>

  <Card title="Tebex Store" icon="cart-shopping" href="https://jaksam1074-fivem-scripts.tebex.io/">
    Achète ou consulte le script sur Tebex.
  </Card>

  <Card title="Langues prises en charge" icon="language">
    Anglais, Tchèque, Français, Allemand, Hongrois, Italien, Portugais, Espagnol
  </Card>
</CardGroup>

## Documentation

<CardGroup cols={3}>
  <Card title="Installation" icon="download" href="/fr/farming-creator/installation">
    Installe et fais fonctionner Farming Creator sur ton serveur.
  </Card>

  <Card title="Référence développeur" icon="code" href="/fr/farming-creator/client">
    Exports, events et référence d'intégration pour les développeurs.
  </Card>
</CardGroup>
