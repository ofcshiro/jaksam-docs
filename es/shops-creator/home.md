---
title: "Shops Creator"
description: "Documentación oficial del script FiveM Shops Creator, incluyendo enlaces al hilo del foro y a la tienda."
icon: "cart-shopping"
---

Esta es la documentación oficial de **Shops Creator**.

<CardGroup cols={2}>
  <Card title="FiveM Forum Post" icon="comments" href="https://forum.cfx.re/t/shops-creator-esx-qbcore-easily-create-in-game-admin-and-players-owned-shops/5036627">
    Hilo oficial de lanzamiento en el foro de FiveM.
  </Card>

  <Card title="Tebex Store" icon="cart-shopping" href="https://jaksam1074-fivem-scripts.tebex.io/">
    Compra o consulta el script en Tebex.
  </Card>

  <Card title="Idiomas soportados" icon="language">
    Inglés, Francés, Alemán, Italiano, Neerlandés, Polaco, Español
  </Card>
</CardGroup>

## Documentación

<CardGroup cols={2}>
  <Card title="Instalación" icon="download" href="/es/shops-creator/installation">
    Instala y pon en marcha Shops Creator en tu servidor.
  </Card>

  <Card title="Guías" icon="book-open" href="/es/shops-creator/how-to-use-images-for-items">
    Usar imágenes para items y otras guías.
  </Card>

  <Card title="Referencia para desarrolladores" icon="code" href="/es/shops-creator/client">
    Exports, events y referencia de integración para desarrolladores.
  </Card>
</CardGroup>
